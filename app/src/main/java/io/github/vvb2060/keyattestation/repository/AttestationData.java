...
public class AttestationData extends BaseData {
    ...
    private byte[] certificateChainEncoded;
    ...
    public Attestation getAttestation() {
        return showAttestation;
    }
    
    public byte[] getCertificateChainEncoded() {
        return certificateChainEncoded;
    }
    ...
    public static AttestationData parseCertificateChain(List<X509Certificate> certs) {
        ...
        // Encode certificate chain to PKCS7 format
        try {
            var factory = CertificateFactory.getInstance("X.509");
            var certPath = factory.generateCertPath(certs);
            data.certificateChainEncoded = certPath.getEncoded("PKCS7");
        } catch (Exception e) {
